...
@testset "FluxSegmentationModels.jl" begin
    ...
    # Test UNet Decoder
    standard_unet_decoder = FluxSegmentationModels.UNetDecoder(UNET_ENCODER_DIMS, UNET_ENCODER_SCALES)
    @test size(standard_unet_decoder(unet_features)) == (IMSIZE,IMSIZE,64,1)

    resnet_unet_decoder = FluxSegmentationModels.UNetDecoder(RESNET50_ENCODER_DIMS, RESNET50_ENCODER_SCALES; decoder_dims=RESNET50_ENCODER_DIMS[1:3])
    @test size(resnet_unet_decoder(resnet_features)) == (IMSIZE÷4,IMSIZE÷4,256,1)
end
